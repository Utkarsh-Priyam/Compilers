...
public class Parser
{
    ...
    {
        if (doneParsing())
            throw new IllegalStateException("Done Parsing File @ Row " + scanner.row + " Col " + scanner.col);

        if (currentToken.equals(currentTokenCheck))
            currentToken = scanner.nextToken();
        else
            throw new ParseErrorException("Parser expected current token '" + currentTokenCheck +
                    "' but found '" + currentToken + "' @ Row " + scanner.row + " Col " + scanner.col);
    }
     ...
     * (i) SWITCH - for switch cases as defined in the class header JavaDoc
     *
    ...
    private Statement parseStatement()
    {
        ...
        switch (currentToken)
        {
            ...
            case "SWITCH":
            {
                eat("SWITCH");
                eat("(");
                Expression switchExp = parseExpression();
                eat(")");
                ...
                while (!currentToken.equals("END"))
                {
                    ...
                    if (currentToken.equals("CASE") || currentToken.equals("DEFAULT"))
                        statements.add(null);
                    else
                        statements.add(parseStatement());
                }
                eat("END");
                eat(";");
                return new Switch(switchExp, expressions, statements);
            }
            default:
            {
                String varName = currentToken;
